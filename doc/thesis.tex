...
% TODO
% Bring in more of earlier EDA doc.
% Tie sections 3 and 4 together. Not made it clear which model chosen, reasons.
% Reformat graphs so titles are clear.
% Summarize section 3 results, fix error.
% Discuss how/if decision rules could be applied to the other models.
% Describe GI algorithm implementation and scaling in more detail.
% General reviewing.

% Last technical element:
% Extend with a bidding stage, even if very simplified problem
...
Where exact tests are not available, we will resort to LR tests and Information criterion approaches.

\section{Model definitions} 

We define a number of models to fit the data with the goal of modelling acquisitions.

We start with simple binomial model (m1), then look to beta binomial (m3) to deal with over-dispersion. We then bring in intermediate clicks, hoping these can offer more information on the likelihood of future acqusitions. We discuss under what conditions clicks provide information on acquisitions in the appendix. We consider both cluster models as well as a correlation models to describe dependency between p and q.
...
Useful if data not completely independent but is over dispersed for a binomial model. Here we assume a hierarchical model where the conversion rate for each arm is sampled from a Beta distribution with common $\alpha$, $\beta$ parameters across all arms. In a sense, this model lies somewhere between model 1 and model 2, in that data about lever 1 gives us some information about lever 2 via inference of $\alpha$, $\beta$ parameters. 

Since acquisitions are rare events, we try to gain more information about a lever be modelling the click-through-rate separately from the conversions. 
...
% TODO - SINCE YOU LATER TREAT EACH ARM INDEPENDENTLY, BE CLEAR WHETHER YOU ARE USING MODEL 2 OR MODEL 3.
...
\begin{description}
	...
	\item[IWC] Impression weighted correlation - a Pearson correlation coefficient weighted by the number of impressions ($n$). This measure is intended to reduce some of the noise coming from the many sites with few impressions.
	\item[logIWCc] Impression weighted correlation between log(CVR) and log(CTR).
...
% TODO - original doc used to create results table?
